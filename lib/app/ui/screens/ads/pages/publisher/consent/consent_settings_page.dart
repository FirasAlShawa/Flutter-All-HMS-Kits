...
class _ConsentSettingsPageState extends State<ConsentSettingsPage> {
  ...
  final Consent consentInfo = Consent.getInstance;
  ...
  void loadBannerAd(ConsentStatus status) async {
    ...
    if (options == null) options = RequestOptions.build();

    options.setTagForUnderAgeOfPromise = UnderAge.promiseTrue;
    options.setNonPersonalizedAd = status.index;

    await HwAds.setRequestOptions(options);
    AdParam adParam = AdParam.build();
